...
export abstract class CommentsApi extends LanguageService {
  private static axios = Axios.create(
    {
      validateStatus: () => true,
      baseURL: "https://localhost:5001/api/v1/admin/admincomments/",
      headers: {
        common: {
  ...
  static async index(jwt: string | null): Promise<ICommentAdminDTO[]> {
    const url = "";
    const response = await this.axios.get<ICommentAdminDTO[]>(url, { headers: { Authorization: 'Bearer ' + jwt, 'accept-language': this.culture } });
  ...
  static async history(id: string, jwt: string | null): Promise<ICommentAdminDTO[]> {
  ...
  static async details(id: string, jwt: string | null): Promise<ICommentAdminDTO> {
    const url = id;
    const response = await this.axios.get<ICommentAdminDTO>(url, { headers: { Authorization: 'Bearer ' + jwt, 'accept-language': this.culture } });
  ...
  static async create(model: ICommentAdminDTO, jwt: string | null): Promise<ResponseDTO> {
    const url = "";
  ...
  static async edit(id: string, model: ICommentAdminDTO, jwt: string | null): Promise<ResponseDTO> {
    const url = id;
