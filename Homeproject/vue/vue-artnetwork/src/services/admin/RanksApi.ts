...
export abstract class RanksApi extends LanguageService {
  private static axios = Axios.create(
    {
      validateStatus: () => true,
      baseURL: apiHost + "/api/v1/admin/adminranks/",
      headers: {
        common: {
  ...
  static async index(jwt: string | null): Promise<IRankAdminDTO[]> {
    const url = "";
    const response = await this.axios.get<IRankAdminDTO[]>(url, { headers: { Authorization: 'Bearer ' + jwt, 'accept-language': this.culture } });
