...
export abstract class GiftsApi extends LanguageService {
  private static axios = Axios.create(
    {
      validateStatus: () => true,
      baseURL: apiHost + "/api/v1/admin/admingifts/",
      headers: {
        common: {
  ...
  static async index(jwt: string | null): Promise<IGiftAdminDTO[]> {
    const url = "";
    const response = await this.axios.get<IGiftAdminDTO[]>(url, { headers: { Authorization: 'Bearer ' + jwt, 'accept-language': this.culture } });
