...
export abstract class ChatRolesApi extends LanguageService {
  private static axios = Axios.create(
    {
      validateStatus: () => true,
      baseURL: apiHost + "/api/v1/admin/adminchatroles/",
      headers: {
        common: {
  ...
  static async index(jwt: string | null): Promise<IChatRoleAdminDTO[]> {
    const url = "";
    const response = await this.axios.get<IChatRoleAdminDTO[]>(url, { headers: { Authorization: 'Bearer ' + jwt, 'accept-language': this.culture } });
  ...
  static async history(id: string, jwt: string | null): Promise<IChatRoleAdminDTO[]> {
  ...
  static async details(id: string, jwt: string | null): Promise<IChatRoleAdminDTO> {
    const url = id;
    const response = await this.axios.get<IChatRoleAdminDTO>(url, { headers: { Authorization: 'Bearer ' + jwt, 'accept-language': this.culture } });
  ...
  static async create(model: IChatRoleAdminDTO, jwt: string | null): Promise<ResponseDTO> {
    const url = "";
  ...
  static async edit(id: string, model: IChatRoleAdminDTO, jwt: string | null): Promise<ResponseDTO> {
    const url = id;
